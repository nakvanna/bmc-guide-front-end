<template>
  <q-dialog v-model="show_dialog" full-width persistent>
    <q-card>
      <q-bar>
        <div class="text-h6">Add New Sponsor</div>
        <q-space />
        <q-btn dense flat icon="close" v-close-popup color="negative">
          <q-tooltip>Close</q-tooltip>
        </q-btn>
      </q-bar>
      <q-form @submit="storeSponsor" @reset="onReset" class="q-gutter-md">
  ...
  methods: {
    ...
    storeSponsor() {
      ...
      if (vm.formData.has("image")) {
        ...
        vm.$q.loading.show();
        vm.$store
          .dispatch("sponsors/storeSponsor", vm.formData)
          .then(function(d) {
            vm.onReset();
